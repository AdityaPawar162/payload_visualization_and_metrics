 ...
 */
PayloadVisualizationAndMetricsNode::PayloadVisualizationAndMetricsNode()
: Node("payload_visualization_and_metrics_node")
{
  // Create subscription to robot_description topic
  subscription_ = this->create_subscription<std_msgs::msg::String>(
    "robot_description",
    rclcpp::QoS(rclcpp::KeepLast(1))
      .transient_local(),  // Use transient local for robot_description
    std::bind(
      &PayloadVisualizationAndMetricsNode::robot_description_callback, this,
      std::placeholders::_1));

  joint_state_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
    "joint_states", 10,
    std::bind(
      &PayloadVisualizationAndMetricsNode::joint_state_callback, this, std::placeholders::_1));
 ...
 */
void PayloadVisualizationAndMetricsNode::robot_description_callback(
...
void PayloadVisualizationAndMetricsNode::joint_state_callback(
  const sensor_msgs::msg::JointState::SharedPtr msg)
{
  ...
  try
  {
    ...
    // Debug output
    RCLCPP_INFO(
      this->get_logger(), "Received joint states with %zu positions", msg->position.size());

    // Update each joint in the model that exists in the message
    for (size_t i = 1; i < njoints_; ++i)
    {  // Skip joint 0 (the universe)
      ...
      if (it != joint_name_to_idx.end())
      {
        ...
        if (
          model_.joints[joint_id].nq() == 1 && joint_config_start < currentQ_.size() &&
...
bool PayloadVisualizationAndMetricsNode::calculateMaxDownwardForce()
{
  ...
  for (int i = 0; i < nv_; ++i)
  {
    ...
    if (tau_needed > tolerance)
    {  // Payload requires positive torque from joint i
    ...
    {  // Payload requires negative torque from joint i (tau_needed < -tolerance)
  ...
  if (min_P_max_i == std::numeric_limits<double>::infinity())
  {
    RCLCPP_WARN(
      this->get_logger(),
      "No joint appears to limit the payload. Calculation might be inaccurate or payload direction "
      "has no effect.");
  ...
  else
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Max Additional Payload Mass (Downward): %.4f kg (Force Limit: %.4f N)",
    ...
    else if (latest_max_payload_mass_ > 0)
    {
      RCLCPP_WARN(
        this->get_logger(),
        "Payload limit found, but couldn't identify a specific limiting joint index.");
    }
  }
...
void PayloadVisualizationAndMetricsNode::extractKinematicsDynamicsProperties()
{
  ...
  for (size_t i = 1; i < njoints_; i++)
  {
    ...
    for (int dof = 0; dof < joint_model.nq(); dof++)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "Joint %zu (%s) DoF %d: [%f, %f]", i, joint_name.c_str(), dof,
        model_.lowerPositionLimit[config_idx], model_.upperPositionLimit[config_idx]);
    }
    ...
    for (int dof = 0; dof < joint_model.nv(); dof++)
    {
      ...
      RCLCPP_INFO(
        this->get_logger(), "Joint %zu (%s) DoF %d velocity limit: %f", i, joint_name.c_str(), dof,
        model_.velocityLimit[velocity_idx]);

      RCLCPP_INFO(
        this->get_logger(), "Joint %zu (%s) DoF %d torque/effort limit: %f", i, joint_name.c_str(),
        dof, model_.effortLimit[velocity_idx]);
    }
  }
}
...
bool PayloadVisualizationAndMetricsNode::getJacobian()
{
  ...
  pinocchio::computeFrameJacobian(
    model_, data_, currentQ_, arm_7_link, pinocchio::LOCAL_WORLD_ALIGNED, J);
  ...
  RCLCPP_INFO(
    ...
    J(0, 10), J(0, 11), J(0, 12), J(0, 13), J(1, 0), J(1, 1), J(1, 2), J(1, 3), J(1, 4), J(1, 5),
    J(1, 6), J(1, 7), J(1, 8), J(1, 9), J(1, 10), J(1, 11), J(1, 12), J(1, 13), J(2, 0), J(2, 1),
    J(2, 2), J(2, 3), J(2, 4), J(2, 5), J(2, 6), J(2, 7), J(2, 8), J(2, 9), J(2, 10), J(2, 11),
    J(2, 12), J(2, 13), J(3, 0), J(3, 1), J(3, 2), J(3, 3), J(3, 4), J(3, 5), J(3, 6), J(3, 7),
    J(3, 8), J(3, 9), J(3, 10), J(3, 11), J(3, 12), J(3, 13), J(4, 0), J(4, 1), J(4, 2), J(4, 3),
    J(4, 4), J(4, 5), J(4, 6), J(4, 7), J(4, 8), J(4, 9), J(4, 10), J(4, 11), J(4, 12), J(4, 13),
    J(5, 0), J(5, 1), J(5, 2), J(5, 3), J(5, 4), J(5, 5), J(5, 6), J(5, 7), J(5, 8), J(5, 9),
    J(5, 10), J(5, 11), J(5, 12), J(5, 13));
